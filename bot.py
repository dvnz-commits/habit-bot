...
def handle(msg):
    ...
    # /set belajar 08:00
        ...
        if len(parts) == 3:
            ...
            if hid not in HABITS:
                send(f"❌ Habit tidak dikenal.\nGunakan salah satu: {', '.join(HABITS.keys())}")
    ...
    # /lihat
        ...
        if not aktif:
            ...
            lines = [f"{HABITS.get(k, k)}: {', '.join(sorted(v))}" for k, v in aktif.items()]
    ...
    # /hapus belajar 08:00
        ...
        if len(parts) == 3:
            ...
            if hid in data and jam in data[hid]:
                data[hid].remove(jam)
                dump(data)
                send(f"🗑️ Pengingat {HABITS.get(hid, hid)} jam {jam} dihapus")
            else:
        ...
        send("Perintah tidak dikenal. Kirim /mulai untuk melihat daftar perintah.")
...
# ── POLLING (terima perintah dari user) ──
def polling():
    offset = None
    while True:
...
# ── SERVER (biar Railway tidak mati) ──
@app.route('/')
def home():
    return '🤖 Habit Tracker Bot sedang berjalan!'
